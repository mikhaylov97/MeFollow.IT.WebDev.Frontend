import {Link} from "react-router-dom";
import React from "react";
import "./common.css";
import UnauthorizedHeader from "../../unauthorized-header";

const RegistrationTypePage = () => {
    return (
        <div className="main-container-jsc-10">
            <div className="container">
                <UnauthorizedHeader />
                <RegistrationTypePageBody />
            </div>
        </div>
    );
};

export default RegistrationTypePage;

const RegistrationTypePageBody = () => {
    return (
        <div className="unauthorized-main-wrapper-jsc-10 relative-jsc-10 row d-flex justify-content-center align-items-center">
            <div className="form-wrapper-jsc-10">
                <div className="mb-4"><p className="title-jsc-10">Регистрация</p></div>
                <div className="align-center-jsc-10 mb-3">
                    <p className="without-margin-jsc-10">Войти с помощью</p>
                </div>
                <div className="align-center-jsc-10 mb-4">
                    <button className="social-icon-jsc-10">
					<span>
						<svg width="56" height="56" viewBox="0 0 56 56" fill="none">
							<rect x="0.5" y="0.5" width="55" height="55" rx="27.5" fill="#fff" stroke="#DFE1E7"></rect>
							<path fillRule="evenodd" clipRule="evenodd"
                                  d="M36.64 28.205c0-.638-.057-1.252-.164-1.841H28v3.481h4.844a4.14 4.14 0 01-1.796 2.717v2.258h2.908c1.702-1.567 2.684-3.874 2.684-6.615z"
                                  fill="#4285F4"></path>
							<path fillRule="evenodd" clipRule="evenodd"
                                  d="M28 37c2.43 0 4.467-.806 5.956-2.18l-2.909-2.259c-.805.54-1.836.86-3.047.86-2.344 0-4.328-1.584-5.036-3.711h-3.007v2.332A8.997 8.997 0 0028 37z"
                                  fill="#34A853"></path>
							<path fillRule="evenodd" clipRule="evenodd"
                                  d="M22.964 29.71a5.41 5.41 0 01-.282-1.71c0-.593.102-1.17.282-1.71v-2.332h-3.007A8.996 8.996 0 0019 28c0 1.452.348 2.827.957 4.042l3.007-2.332z"
                                  fill="#FBBC05"></path>
							<path fillRule="evenodd" clipRule="evenodd"
                                  d="M28 22.58c1.321 0 2.508.454 3.44 1.346l2.582-2.582C32.462 19.892 30.426 19 28 19a8.997 8.997 0 00-8.043 4.958l3.007 2.332c.708-2.127 2.692-3.71 5.036-3.71z"
                                  fill="#EA4335"></path>
						</svg>
					</span>
                    </button>
                    <button className="social-icon-jsc-10">
					<span>
						<svg width="56" height="56" viewBox="0 0 56 56" fill="none">
							<rect width="56" height="56" rx="28" fill="#5181B8"></rect>
							<path d="M38.04 23.11c-.08.39-.392 1.028-.938 1.913a58.697 58.697 0 01-1.446 2.344c-.442.651-.677.99-.703 1.016-.156.234-.234.404-.234.508 0 .104.078.286.234.547l.234.273.626.625c.234.234.494.508.78.82.287.313.548.625.782.938.26.286.482.586.664.898.208.313.352.573.43.781.182.651-.078.977-.782.977h-2.304a1.26 1.26 0 01-.469-.078 1.66 1.66 0 01-.43-.274c-.156-.13-.3-.273-.43-.43-.13-.156-.325-.377-.585-.663-.26-.287-.521-.56-.782-.82-1.041-.99-1.81-1.485-2.304-1.485-.313 0-.495.091-.547.273-.052.183-.078.704-.078 1.563.026.417.039.755.039 1.016 0 .312-.104.546-.313.703-.208.13-.664.195-1.367.195-1.224 0-2.46-.365-3.71-1.094-1.25-.729-2.345-1.77-3.282-3.125a25.642 25.642 0 01-2.227-3.554c-.572-1.198-.95-2.11-1.132-2.735-.183-.625-.274-1.068-.274-1.328 0-.443.26-.664.781-.664h2.305c.287 0 .495.065.625.195.156.13.287.365.39.703.548 1.589 1.212 3.008 1.993 4.258.807 1.25 1.42 1.875 1.836 1.875.156 0 .26-.052.312-.156.078-.13.118-.365.118-.703v-3.399a3.134 3.134 0 00-.196-.937c-.104-.26-.208-.443-.312-.547l-.313-.39a.7.7 0 01-.117-.352c0-.104.026-.195.078-.274a.698.698 0 01.195-.195.693.693 0 01.313-.078h3.633c.416 0 .625.287.625.86v4.53c0 .443.117.665.351.665.26 0 .625-.235 1.094-.703.495-.547.977-1.211 1.445-1.993.47-.78.82-1.432 1.055-1.953l.352-.742c.182-.443.508-.664.976-.664h2.305c.625 0 .86.287.703.86z"
                                  fill="#fff"></path>
						</svg>
					</span>
                    </button>
                </div>
                <div className="divider-jsc-10 align-center-jsc-10 mb-4">
                    <p><span>Или</span></p>
                </div>
                <div className="d-flex align-center-jsc-10 mb-4">
                    <Link className="link-btn-jsc-10" to="/registration">Регистрация с эл. адресом</Link>
                </div>
                <div className="align-center-jsc-10 mb-4">
                    <p className="comment-jsc-10">Регистрируясь через Google, Facebook, VK или через электронную почту, вы
                        соглашаетесь с текстом <Link className="simple-link-jsc-10" to="/policy">Политики в отношении обработки и
                            защиты персональных данных</Link>.</p>
                </div>
                <div className="align-center-jsc-10">
                    <p className="without-margin-jsc-10">Уже есть аккаунт? <Link className="strong-link-jsc-10"
                                                                       to="/login">Войти</Link></p>
                </div>
            </div>
        </div>
    );
};
